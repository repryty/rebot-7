...
async def execute(client_data: ClientData):
    ...
    if model_name:
        client_data.guild_genai_config.model=model_name
        ...
        gemini_model_list = ["gemini-2.0-flash", "gemini-2.0-flash-lite"]+[i.name[7:] for i in client_data.genai_client.models.list() if ("generateContent" in i.supported_actions) and ("exp" in i.name)]
